...
def collect_mbpo_rollout(
    ...
    """
    Simulating a trajectory entirely using our learned models (not the real environment)

    SAC replay buffer is separated from the replay buffer.
    replay_buffer includes real environment data only as Dynamics models must learn
    from real data only, while sac_replay_buffer contains real and synthetic data,
    then SAC benefits from both real and synthetic experience.

    In MBPO, MB agent does not act, but only generates synthetic data. It
    predicts dynamics to "imagine" what would happen and help SAC learn faster with
    less real data.
    """
    obs, acs, rewards, next_obs, dones = [], [], [], [], []
    for _ in range(rollout_len):
        # (8) TODO(student): collect a rollout using the learned dynamics models
        ...
        for i in range(mb_agent.ensemble_size):
            next_ob_i = mb_agent.get_dynamics_predictions(i, ob[None, :], ac[None, :])
            next_ob_i = next_ob_i[0]
            next_ob_list.append(next_ob_i)
...
def run_training_loop(
    ...
    assert not discrete, "Our MPC implementation only supports continuous action spaces."
    ...
    if sac_config is not None:
        ...
        # * KEY: Switch the role of the actor.
    ...
    for itr in range(config["num_iters"]):
        ...
        Traditional RL Problem: Learning requires LOTS of real environment 
        interactions (expensive/slow)
        
        Model-Based Solution:
        Collect data with random policy → get (s, a, s')
        Train dynamics model f(s,a) to predict s' using all data collected so far
        Use model to plan actions via MPC
        Collect MORE data with MPC policy then retrain
        """
        if itr == 0:
            # (4) TODO(student): collect at least config["initial_batch_size"] transitions
            ...
            # (4) TODO(student): collect at least config["batch_size"] transitions
            # with our `actor_agent`
            trajs, envsteps_this_batch = utils.sample_trajectories(
        ...
        # insert newly collected on-policy data into replay buffer
        """
        Aggregates the collected data into a replay buffer with the current 
        MPC policy and retrains the model with all data collected so far.
        """
        for traj in trajs:
        ...
        for _ in tqdm.trange(config["num_agent_train_steps_per_iter"], dynamic_ncols=True):
            step_losses = []
            # (5) TODO(student): train the dynamics models
            # HINT: train each dynamics model in the ensemble with a *different*
            # batch of transitions!
            # Use `replay_buffer.sample` with config["train_batch_size"].
            for model_i in range(mb_agent.ensemble_size):
