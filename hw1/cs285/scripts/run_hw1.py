...
def run_training_loop(params):
    ...
    # (2) TODO: Implement missing functions in this class.
    actor = MLPPolicySL(
        ac_dim,
        ob_dim,
        params["n_layers"],
    ...
    for itr in range(params["n_iter"]):
        ...
        _logger.info("  Collecting data to be used for training...")
        ...
        if itr == 0:
            ...
            # (4) TODO: collect `params['batch_size']` transitions
            # HINT: use utils.sample_trajectories
            # (4) TODO: implement missing parts of utils.sample_trajectory
            ...
            if params["do_dagger"]:
                ...
                # (4) TODO: relabel collected obsevations (from our policy) with labels from expert policy
                # HINT: query the policy (using the get_action function) with paths[i]["observation"]
                #       and replace paths[i]["action"] with these expert labels
                #
                # Aggregate the data (actions from the expert policy) with the existing datasets.
                # Keep the observations from where my policy went, but label them with what
                # the expert would have done.
                # i. Dataset lose IID, which is intentional.
                #    It is already broken at test time, so DAGGER just makes
                #    training match test-time distribution.
                # ii. Peeking the answer? Yes.
                #     The expert is not guiding execution, only labeling visited states.
                #     DAGGER changes where the data comes from, not what the labels mean.
                # iii. DAgger assumes a reasonably competent expert.
                #      If the expert is bad in rare states, then learner inherits those failures.
                #      Variants:
                # DAgger + RL fine-tuning
                # Confidence-weighted DAgger, Query expert only when learner is uncertain.
                # Safe DAgger / Intervention-based
                # Learned critics / Q-filtering
        ...
        _logger.info("  Training agent using sampled data from replay buffer...")

        training_logs = []
        for _ in range(params["num_agent_train_steps_per_iter"]):
            # (1) TODO: sample some data from replay_buffer
            # HINT1: how much data = params['train_batch_size']
            # HINT2: use np.random.permutation to sample random indices
            # HINT3: return corresponding data points from each array (i.e., not different indices from each array)
            #        for imitation learning, we only need observations and actions.
            sample_data_size = params["train_batch_size"]
            buffer_size = replay_buffer.__len__()
            sample_indices = np.random.permutation(buffer_size)[:sample_data_size]

            _logger.debug("SAMPLE INDICES:", sample_indices)

            ob_batch, ac_batch = (
                replay_buffer.obs[sample_indices],
                replay_buffer.acs[sample_indices],
            )
