...
class RNDAgent(DQNAgent):
    def __init__(
        ...
        **kwargs
        ...
        # To prevent the neural networks from having zero prediction error right
        # from the beginning,
    ...
    def update_rnd(self, obs: torch.Tensor) -> torch.Tensor:
        ...
        When the randomly chosen vector-valued function trained, RND would
        train another network to match the predictions of the previous function
        under the distribution of datapoints in the buffer.
        """
        # (3) TODO(student): update the RND network

        preds = self.rnd_net(obs)
        # The target us frozen
        with torch.no_grad():
            targets = self.rnd_target_net(obs)

        # We still want to backpropgate through self.rnd_net
    ...
    def update(
        ...
        with torch.no_grad():
            # (2) TODO(student): Compute RND bonus for batch and modify rewards
            """
            The RND error (L2 norm) is based on the next observations which
            measures the novelty of the next state.
            """
            preds = self.rnd_net(next_observations)
            targets = self.rnd_target_net(next_observations)
            assert preds.ndim == 2
            rnd_error = torch.norm(preds - targets, dim=1)
