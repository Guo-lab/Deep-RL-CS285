...
def basic_dqn_config(
    ...
    **kwargs
    ...
    print(f"  {'env_name':30s}: {env_name}")
    print(f"  {'total_steps':30s}: {total_steps}")
    print(f"  {'batch_size':30s}: {batch_size}")
    print(f"  {'hidden_size':30s}: {hidden_size}")
    print(f"  {'num_layers':30s}: {num_layers}")
    print(f"  {'learning_rate':30s}: {learning_rate}")
    print("=" * 80 + "\n")
