...
class ReplayBuffer:
    ...
    def load_dataset(self, dataset):
        ...
        n_obs = dataset.next_observations[0]
        ...
        if self.observations is None:
            ...
            self.actions = np.empty((self.max_size, *action.shape), dtype=action.dtype)
            self.rewards = np.empty((self.max_size, *reward.shape), dtype=reward.dtype)
            self.next_observations = np.empty((self.max_size, *n_obs.shape), dtype=n_obs.dtype)
            self.dones = np.empty((self.max_size, *done.shape), dtype=done.dtype)

        # Copy data from dataset - create explicit copies to avoid broadcasting issues
        for i in range(dataset_size):
            self.observations[i] = dataset.observations[i]
            self.actions[i] = dataset.actions[i]
            self.rewards[i] = dataset.rewards[i]
            self.next_observations[i] = dataset.next_observations[i]
            self.dones[i] = dataset.dones[i]
