...
A common way of doing exploration is to visit states with a large prediction error of some quantity, for instance, the TD error or even random functions. The RND algorithm, as covered in Lecture 13, \colorbox{yellow}{aims at encouraging exploration by} 
\colorbox{yellow}{asking the exploration policy to more frequently undertake transitions where the prediction error of a random neural}
\colorbox{yellow}{ network function is high}. Formally, let ${f}_\theta^*(s')$ be a randomly chosen vector-valued function represented by a neural network. RND trains another neural network, $\hat{f}_\phi(s')$ to match the predictions of $f^*_\theta(s')$ under the distribution of datapoints in the buffer, as shown below:
\begin{equation}
    \phi^* = \arg \min_\phi~ \mathbb{E}_{s, a, s' \sim \mathcal{D}} \left[\underbrace{||\hat{f}_\phi(s') - f^*_\theta(s')||}_{\mathcal{E}_\phi(s')} \right].
\end{equation}
If a transition $(s, a, s')$ is in the distribution of the data buffer, the prediction error $\mathcal{E}_\phi(s')$ is expected to be small. On the other hand, for all unseen state-action tuples it is expected to be large. To utilize this prediction error as a reward bonus for exploration, RND trains two critics -- an \emph{exploitation critic}, $Q_R(s, a)$, and an \emph{exploration critic}, $Q_\mathcal{E}(s, a)$, \colorbox{yellow}{where the exploitation critic estimates the return of the policy under} \\
\colorbox{yellow}{the actual reward function and the exploration critic estimates the return of the policy under the reward bonus.} In practice, we normalize error before passing it into the exploration critic, as this value can vary widely in magnitude across states leading to poor optimization dynamics.
...
For the first portion of the offline RL part of this assignment, we will implement the conservative Q-learning (CQL) algorithm. The goal of CQL is to \colorbox{yellow}{prevent overestimation of the policy value}. In order to do that, a conservative, lower-bound Q-function is learned by \colorbox{yellow}{additionally minimizing Q-values alongside a standard}
\colorbox{yellow}{ Bellman error objective}. This is done by augmenting the Q-function training with a regularizer that minimizes the soft-maximum of the Q-values $\log \left( \sum_{a} \exp(Q(s, a)) \right)$ and maximizes the Q-value on the state-action pair seen in the dataset, $Q(s, a)$. The overall CQL objective is given by the standard TD error objective augmented with the CQL regularizer weighted by $\alpha$: $\alpha \left[\frac{1}{N}\sum_{i=1}^N \left(\log\left(\sum_{a} \exp(Q(s_i, a))\right) - Q(s_i, a_i) \right) \right]$. You will tweak this value of $\alpha$ in later questions in this assignment.  
...
This update is similar to weighted behavior cloning (which it resolves to if the Q function is degenerate). But with a well-formed Q estimate, we \colorbox{yellow}{ weight the policy towards selecting actions that are high under our}
\colorbox{yellow}{ learnt q function}. In the update above, the agent \colorbox{yellow}{ regresses onto high-advantage actions} with a large weight, while almost ignoring low-advantage actions. This actor update amounts to weighted maximum likelihood (i.e., supervised learning), where the targets are obtained by re-weighting the state-action pairs observed in the current dataset
...
The \colorbox{yellow}{actor update is decoupled from the critic update} (hence \textit{implicit} $Q$-learning), and is learned with the same objective as AWAC:
\begin{equation}
...
% The first part in this assignment is to implement a working version of Random Network Distillation. 
The default code will run the \texttt{easy} environment with reasonable hyperparameter settings. Look for the \texttt{\# TODO(student)} markers in the files listed above for detailed implementation instructions.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../exploration_visualization/PointmassEasy-v0_random.png}
        \subcaption{\footnotesize Easy environment}
    \end{minipage}
    \hfill
    \begin{minipage}{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../exploration_visualization/PointmassMedium-v0_random.png}
        \subcaption{\footnotesize Medium environment}
    \end{minipage}
    \hfill
    \begin{minipage}{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../exploration_visualization/PointmassHard-v0_random.png}
        \subcaption{\footnotesize Hard environment}
    \end{minipage}
    \caption{\small Random policy exploration on three environments showing limited state coverage.}
    \label{fig:random_exploration}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../exploration_visualization/PointmassEasy-v0_rnd1.0.png}
        \subcaption{\footnotesize Easy environment}
    \end{minipage}
    \hfill
    \begin{minipage}{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../exploration_visualization/PointmassMedium-v0_rnd1.0.png}
        \subcaption{\footnotesize Medium environment}
    \end{minipage}
    \hfill
    \begin{minipage}{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../exploration_visualization/PointmassHard-v0_rnd1.0.png}
        \subcaption{\footnotesize Hard environment}
    \end{minipage}
    \caption{\small RND exploration showing improved state coverage with low error in reachable states and high error in walls.}
    \label{fig:rnd_exploration}
\end{figure}
...
\subsection{CQL}
Now that we have implemented RND for collecting exploration data that is (likely) useful for performing exploitation, we will perform offline RL on this dataset and see \colorbox{cyan}{how close the resulting policy} 
\colorbox{cyan}{is to the optimal policy}. To begin, you will implement the conservative Q-learning algorithm in this \verb+cs285/agents/cql_agent.py+.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/cql_dqn_q_values.png}
        \subcaption{\footnotesize Q-values during training}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/cql_dqn_return.png}
        \subcaption{\footnotesize Evaluation returns}
    \end{minipage}
    \caption{\small CQL vs DQN comparison on Easy and Medium environment. CQL lead to more conservative Q-value estimates than DQN, reducing overestimation.}
    \label{fig:cql_dqn_comparison}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/cql_alpha_q_val.png}
        \subcaption{\footnotesize Q-values with different $\alpha$}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/cql_alpha_return.png}
        \subcaption{\footnotesize Evaluation returns with different $\alpha$}
    \end{minipage}
    \caption{\small Effect of CQL $\alpha$ parameter on Medium environment. Q-values exhibit a U-shaped curve: decreasing from $\alpha=0$ to $\alpha=1$ (increasing conservatism), then increasing from $\alpha=1$ to $\alpha=10$ (over-regularization causes the network to compensate against excessive penalty, destabilizing learning). The minimum Q-values at $\alpha \approx 1$ represent the most conservative estimates. However, evaluation returns are best at $\alpha=0$ (DQN) and degrade with all non-zero $\alpha$ values, indicating that conservatism sacrifices performance when the offline dataset quality is already sufficient.}
    \label{fig:cql_alpha_sweep}
\end{figure}

\newpage
\vspace{-0.3em}
\subsection{Policy Constraint Methods: IQL and AWAC}
While CQL learns an actor via a modification to actor-critic algorithms like DQN that regularizes actions towards those found in the dataset by decreasing OOD $Q$-values, AWAC \colorbox{cyan}{learns an in-distribution policy }
\colorbox{cyan}{directly by performing weighted behavior cloning on the dataset}.
...
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figures/easy_dqn_cql_awac_iql_return.png}
    \caption*{Evaluation Return}
\end{minipage}
\hfill
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figures/easy_dqn_cql_awac_iql_q_val.png}
    \caption*{Q Values}
\end{minipage}
\vspace{-0.3em}
\caption{Comparison of DQN, CQL, AWAC, and IQL on Easy Maze}
\label{fig:easy_comparison}
\end{figure}
...
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figures/medium_dqn_cql_awac_iql_return.png}
    \caption*{Evaluation Return}
\end{minipage}
\hfill
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figures/medium_dqn_cql_awac_iql_q_val.png}
    \caption*{Q Values}
\end{minipage}
\vspace{-0.3em}
\caption{Comparison of DQN, CQL, AWAC, and IQL on Medium Maze}
...
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figures/awac_iql_return.png}
    \vspace{-1.3em}
    \caption*{Evaluation Return}
\end{minipage}
\hfill
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figures/awac_iql_q_val.png}
    \vspace{-1.3em}
    \caption*{Q Values}
\end{minipage}
\vspace{-0.3em}
\caption{AWAC vs IQL Comparison across Easy, Medium, and Hard Mazes}
\label{fig:awac_iql_comparison}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.5\textwidth]{figures/awac_iql_advantage.png}
\vspace{-0.3em}
\caption{Advantage values for AWAC and IQL across Easy, Medium, and Hard Mazes}
\label{fig:awac_iql_advantage}
\end{figure}

\vspace{-0.3em}

\textbf{Key Observations and Analysis:} {\footnotesize
    (The following patterns are consistent with theory and serve as implementation sanity checks.)
}

\textbf{Return Ranking.} On maze tasks, we observe IQL $<$ AWAC $<$ CQL in evaluation return. 
This is expected because maze solving requires long-horizon reasoning and exploration beyond 
the dataset's dominant behavior. 
IQL is extremely conservative, AWAC provides moderate improvement within the dataset 
distribution, while CQL aggressively pushes up the value of dataset actions. 
In mazes, conservatism hurts performance because optimal paths are rare in the dataset, 
requiring stitching together unlikely action sequences.

\textbf{Advantage Statistics.} IQL exhibits significantly negative advantages 
($A \ll 0$), while AWAC and CQL show advantages centered near 0. 
This occurs because IQL trains $V(s)$ as a high expectile ($\tau \approx 0.7$-$0.9$), 
making it optimistic. 
For most dataset actions, $A(s,a) = Q(s,a) - V(s) < 0$. 
This design makes IQL's actor behave like behavioral cloning, 
\textbf{weighting only top-quality actions}. 
In contrast, AWAC's $V(s) = \mathbb{E}_\pi[Q(s,a)]$ naturally centers advantages around 0, 
providing balanced improvement signals.

\textbf{Q-Value Ordering.} Counterintuitively, AWAC has the lowest Q-values, 
CQL shows medium Q-values, but IQL has the highest Q-values. 
This does not contradict IQL's conservatism, \emph{conservatism is determined by} $Q - V$, 
\emph{not absolute Q magnitude}. 
IQL's high Q-values come from training against $V(s')$ targets, 
where $V$ is optimistically high via expectile regression. 
The actor only sees $Q - V$, which remains small or negative, 
resulting in conservative behavior despite high absolute Q-values. 
AWAC uses policy-based targets producing lower but consistent Q-values, 
while CQL's explicit conservative penalty (penalizing OOD actions while elevating 
dataset actions) produces intermediate Q-values.

% \textbf{Takeaway.} Offline RL performance is not correlated with Q-value magnitude—what matters is relative action ordering and advantage signal quality. IQL is conservative because it makes $V$ almost as large as $Q$, not because it makes $Q$ small.
...
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{../exploration_visualization/PointmassMedium-v0_rnd1.000001.png}
    \caption*{1000 steps}
\end{minipage}
\hfill
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{../exploration_visualization/PointmassMedium-v0_rnd1.0000001.png}
    \caption*{5000 steps}
\end{minipage}
...
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{../exploration_visualization/PointmassMedium-v0_rnd1.00000001.png}
    \caption*{10000 steps}
\end{minipage}
\hfill
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{../exploration_visualization/PointmassMedium-v0_rnd1.000000001.png}
    \caption*{20000 steps}
\end{minipage}
\caption{RND Exploration on Medium Maze with Different Dataset Sizes}
\label{fig:rnd_medium_ablation}
\end{figure}
...
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figures/medium_cql_abla_return.png}
    \caption*{Evaluation Return}
\end{minipage}
\hfill
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figures/medium_cql_abla_q.png}
    \caption*{Q Values}
\end{minipage}
\vspace{-0.3em}
\caption{CQL Performance with Different Dataset Sizes on Medium Maze}
\label{fig:cql_data_ablation}
\end{figure}

\vspace{-0.3em}

\textbf{Data Ablation Analysis:} More data improves sample efficiency and reduces distributional shift. For CQL, larger datasets reduce the need for aggressive pessimism, enabling stronger policy improvement with more reliable Q-value estimates. % For IQL (not shown), more data tightens the expectile regression and shrinks advantage magnitudes, as the high-expectile baseline $V(s)$ becomes better calibrated with richer coverage.
...
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figures/Screenshot from 2026-01-11 22-55-10.png}
    \caption*{Evaluation Return}
\end{minipage}
\hfill
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figures/Screenshot from 2026-01-11 22-55-25.png}
    \caption*{Q Values}
\end{minipage}
\vspace{-0.3em}
\caption{Online fine-tuning results on PointmassHard-v0 environment for different offline RL algorithms (AWAC, CQL, IQL, and DQN). The 100k indicates the transition from offline training to online fine-tuning. After the transition, all algorithms show rapid improvement in evaluation returns, with DQN achieving the highest performance (-40 to -60 range). Q-values stabilize after fine-tuning begins.}
\label{fig:online_finetuning}
\end{figure}

Fine-tuning further improves further after offline RL converged to be stable.
