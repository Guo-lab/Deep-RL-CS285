...
def run_training_loop(args):
    ...
    for itr in range(args.n_iter):
        ...
        # (8) TODO: sample `args.batch_size` transitions using utils.sample_trajectories
        ...
        # trajs should be a list of dictionaries of NumPy arrays, where each dictionary
        # corresponds to a trajectory.
        # this line converts this into a single dictionary of lists of NumPy arrays.
        trajs_dict = {k: [traj[k] for traj in trajs] for k in trajs[0]}
        if itr == 0:
            """
            INFO:__main__:Sampled 37 trajectories
            INFO:__main__:Trajs dict keys: dict_keys(['observation', 'image_obs', 'reward', 'action', 'next_observation', 'terminal'])
            INFO:__main__:First trajectory obs shape: (41, 4)
            INFO:__main__:First trajectory action shape: (41,)
            INFO:__main__:First trajectory reward shape: (41,)
            INFO:__main__:First trajectory length: 41
            """
            _logger.info(f"Sampled {len(trajs)} trajectories")
            _logger.info(f"Trajs dict keys: {trajs_dict.keys()}")
            _logger.info(f"First trajectory obs shape: {trajs[0]['observation'].shape}")
            _logger.info(f"First trajectory action shape: {trajs[0]['action'].shape}")
            _logger.info(f"First trajectory reward shape: {trajs[0]['reward'].shape}")
            _logger.info(f"First trajectory length: {len(trajs[0]['reward'])}")

        # (8) TODO: train the agent using the sampled trajectories and the agent's update function
