...
def sample_trajectory(
    ...
    while True:
        ...
        # (1) TODO use the most recent ob and the policy to decide what to do
        ob_batch = np.expand_dims(ob, axis=0)
        ac: np.ndarray = policy.get_action(np.array(ob_batch))
        ac = ac[0]

        # (1) TODO: use that action to take a step in the environment
        next_ob, rew, done, _ = env.step(ac)

        # (1) TODO rollout can end due to done, or due to max_length
        steps += 1
